...
export function voiceWorkflowEmail(p: VoiceWorkflowPayload): { subject: string; html: string } {
  ...
  if (type === 'final_ready') {
    // EU AI Act 50(4):AI / hybrid 交付檔要告知客戶(=發布方)的揭露義務 + 附現成口播稿;真人案(live)不加。
    const aiDisclosureBlock = mode === 'live' ? '' : `
        <p style="color:#9ca3af;font-size:13px;line-height:1.7;margin:16px 0 0;padding-top:14px;border-top:1px solid rgba(255,255,255,0.08);">${tx(
          'AI 內容揭露提醒。本檔為 AI 生成語音。依歐盟 AI 法第 50 條,若您在歐盟境內發布或散布本內容,您(作為發布方)須向受眾揭露內容為 AI 生成。常見做法:①內容開頭加一句口播揭露 ②影片片尾字卡或說明欄標示 ③播放器旁顯示「AI」標籤。屬廣告、遊戲、有聲書等創作型作品者,可採不妨礙作品呈現的方式揭露(如片尾或說明欄),不必將揭露錄進作品本身。本檔已內嵌 C2PA 內容憑證,請勿移除。',
          'AI 内容披露提醒。本文件为 AI 生成语音。根据欧盟《人工智能法》第 50 条,若您在欧盟境内发布或传播本内容,您(作为发布方)须向受众披露内容为 AI 生成。常见做法:①内容开头加一句语音披露 ②视频片尾字卡或说明栏标示 ③播放器旁显示「AI」标签。属广告、游戏、有声书等创作类作品的,可采用不妨碍作品呈现的方式披露(如片尾或说明栏),无需将披露录入作品本身。本文件已内嵌 C2PA 内容凭证,请勿移除。',
          'AI content disclosure notice. This file contains AI-generated speech. Under Article 50 of the EU AI Act, if you publish or distribute this content in the EU, you — as the publisher — must disclose to your audience that it is AI-generated. Common approaches: (1) a short spoken disclaimer at the start, (2) an end-card or description-field label for video, (3) an "AI" badge next to the player. For evidently artistic or creative works (advertising, games, audiobooks), disclosure may be made in a manner that does not hamper enjoyment of the work — e.g. in end credits or accompanying notes — rather than recorded into the work itself. This file carries an embedded C2PA content credential; please do not remove it.')}</p>
        <p style="color:#9ca3af;font-size:13px;line-height:1.7;margin:8px 0 0;">${tx(
          '若您選擇口播揭露,可直接使用:短版「本段語音由 AI 生成。」/完整版「以下語音內容由人工智慧生成,非真人錄音。」',
          '若您选择语音披露,可直接使用:短版「本段语音由 AI 生成。」/完整版「以下语音内容由人工智能生成,非真人录音。」',
          'If you opt for a spoken disclaimer, feel free to use: short — "This audio is AI-generated." / full — "The following audio was generated by artificial intelligence and is not a human recording."')}</p>`;
    const content = `
      ...
      ${bodyCard(ord, `<p style="color:#d1d5db;font-size:15px;line-height:1.7;margin:0;">${tx(
        ...
        `Your voiceover for ${ord} is finalized. All deliverable files are ready to download in your dashboard. Thank you for choosing Onyx Studios.`)}</p>${aiDisclosureBlock}`)}
...
${btn('Audition here')}
<p>To address the questions we hear most, upfront:</p>
