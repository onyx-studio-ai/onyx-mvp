...
export const CONTRACT_VERSION = 'v1.0-2026-07-19';
...
5. REVENUE SHARE. For every generation using Licensor's AI Twin, Licensor receives twenty-five percent (25%) of the Tier 1 list price corresponding to the usage volume, regardless of the plan actually purchased by the client. Details are itemized in the talent portal; settlement follows Platform's standard talent-payout mechanism. Licensor bears their own taxes. The 25% ratio is unaffected by Platform price adjustments.

6. TRAINING MATERIALS AND SUPPLEMENTARY RECORDING. Licensor records per Platform's scripts and specifications (quiet environment, professional home-studio equipment, WAV 48kHz/24-bit). Non-conforming materials may be rejected for re-recording. To continuously improve the AI Twin, Licensor agrees to cooperate with supplementary recording upon Platform's invitation; such materials fall under this same Agreement. Compensation for hour-scale corpus recording shall be separately agreed.

6-bis. PATCH RECORDING. Where a client requests localized re-recording, Platform may invite Licensor first (compensated at Platform's standard operational rates); Licensor may decline. If declined or unavailable, Platform may complete the patch via AI alternatives, including voice-conversion technology rendered in the AI Twin's timbre.
...
11. CONFIDENTIALITY. Both parties shall keep revenue-share details and non-public business information confidential.
...
5. 分潤:每筆生成使用,授權人分得該使用量對應 Tier 1 公告牌價之 25%,不論客戶購買何種方案;後台逐筆可查,結算依平台人才分潤機制,稅務自負;平台調價不影響 25% 比例。

6. 語料與補錄:依平台稿件及規格錄製(安靜環境、專業宅錄設備、WAV 48kHz/24bit),不符者退件重錄。授權人同意於平台邀請時配合補錄,適用同一授權與分潤;小時級語料工酬另議。

6-1. 補丁錄音:客戶請求局部補錄時,平台得優先邀請授權人(依標準費率有償),授權人得婉拒;婉拒或不可用時,平台得以 AI 替代方案(含音色轉換技術)完成。
