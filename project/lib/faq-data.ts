// 平台 FAQ 唯一資料源 —— FAQ 頁(app/[locale]/faq/page.tsx)與右下角 HelpWidget 共用。
// getClientFaqs = 客戶向 7 類(搬自原 FAQ 頁,維持三語與內容不變);
// getTalentFaqs = 配音員向(Wing 2026-08-05 拍板)。改內容只改這裡,兩處同步。
export type FaqTx = (tw: string, cn: string, en: string) => string;
export interface FaqItem { q: string; a: string }
export interface FaqCategory { category: string; items: FaqItem[] }

export function getClientFaqs(tx: FaqTx): FaqCategory[] {
  return [
...
export function getTalentFaqs(tx: FaqTx): FaqCategory[] {
  return [
    {
      category: tx('配音員 / 應徵', '配音员 / 应征', 'For Voice Actors'),
      items: [
        {
          q: tx('我要怎麼加入成為配音員？', '我要怎么加入成为配音员？', 'How do I join as a voice actor?'),
          a: tx(
            '到 onyxstudios.ai/apply/voice 填寫申請並附上你的 demo。我們的製作團隊會審核你的聲音與資歷，通過後會為你建立配音員帳號並寄出設定連結，你就能登入後台、完善個人檔案與試聽,開始接案。',
            '到 onyxstudios.ai/apply/voice 填写申请并附上你的 demo。我们的制作团队会审核你的声音与资历，通过后会为你建立配音员账号并寄出设置链接，你就能登录后台、完善个人档案与试听，开始接案。',
            'Apply at onyxstudios.ai/apply/voice and include your demo. Our production team reviews your voice and background; once approved, we create your talent account and email you a setup link so you can log in, complete your profile and demos, and start receiving jobs.'
          ),
        },
        {
          q: tx('有案子的時候我怎麼知道？接案流程是什麼？', '有案子的时候我怎么知道？接案流程是什么？', 'How will I know when there is a job? What is the process?'),
          a: tx(
            '當有適合你的案件時，我們會直接發出邀請通知你。流程是:收到邀請 → 依案件說明試音(有指定稿件就依稿試音;沒有的話用你的 demo 應徵)→ 客戶選定得標 → 你錄製並上傳交件。整個過程都在平台後台完成。',
            '当有适合你的项目时，我们会直接发出邀请通知你。流程是:收到邀请 → 依项目说明试音(有指定稿件就依稿试音;没有的话用你的 demo 应征)→ 客户选定得标 → 你录制并上传交件。整个过程都在平台后台完成。',
            'When a job suits you, we send you an invitation directly. The flow is: receive the invitation → audition per the brief (record to the provided script if there is one; otherwise apply with your demo) → the client selects the winner → you record and upload your delivery. It all happens inside the platform.'
          ),
        },
        {
        ...
        },
        {
          ...
          a: tx(
            ...
            '在后台填写你的收款资料即可。款项经核准拨付后，约 7 个工作日内汇出、陆续入账。台湾配音员一般以银行汇款支付;海外配音员可用 PayPal 或国际电汇。每次拨款都会附上拨款证明供你留存。',
            'Enter your payout details in your dashboard. Once a payment is approved and released, it is issued within about 7 working days. Voice actors in Taiwan are typically paid by bank transfer; overseas talent can be paid via PayPal or international wire. Every payout comes with a reference for your records.'
          ),
        },
        {
          q: tx('報酬是抽成還是買斷？怎麼算？', '报酬是抽成还是买断？怎么算？', 'Is pay commission-based or a buyout? How is it calculated?'),
          a: tx(
            '依合作方案而定 —— 不同案件與合作形式有不同的報酬方式。實際條件會在案件說明或邀請時清楚告知，接案前你都會先看到。',
            '依合作方案而定 —— 不同项目与合作形式有不同的报酬方式。实际条件会在项目说明或邀请时清楚告知，接案前你都会先看到。',
            'It depends on the engagement — different projects and arrangements use different compensation models. The exact terms are stated clearly in the brief or invitation, so you always see them before taking a job.'
          ),
        },
      ],
    }
