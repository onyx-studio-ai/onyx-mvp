...
export default function GuestCasting() {
  ...
  const load = useCallback(async () => {
    ...
    setBrief(j.brief); setDealCurrency(j.dealCurrency || null); setCounts(j.roleCounts || {}); setMine(j.myAuditions || []); setClosed(!!j.closed); setAssignedRoles(j.assignedRoles || []);
    setPhase('ready');
  }, [token]);
  useEffect(() => { load(); }, [load]);
    ...
    <Shell>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mb-3">
        {[
          ...
          { l: tx('試音截止', 'Audition due'), v: brief.audition_deadline ? `${brief.audition_deadline}${brief.audition_deadline_time ? ' ' + brief.audition_deadline_time : ''}(${tzLabel(brief.timezone || 'Asia/Taipei', locale)})` : tx('待定', 'TBD') },
          { l: tx('交付截止', 'Delivery'), v: brief.deadline ? `${brief.deadline}${brief.deadline_time ? ' ' + brief.deadline_time : ''}(${tzLabel(brief.timezone || 'Asia/Taipei', locale)})` : tx('待定', 'TBD') },
        ...
        <>
          ...
          <div className="space-y-3">
            {(brief.roles || []).map((ro, i) => (
              <GuestRole key={i} token={token} role={ro} dealCurrency={dealCurrency} count={counts[ro.name || ''] || 0}
  ...
  return (
    <div className="border border-rose-400/30 bg-rose-500/[0.06] rounded-lg p-3">
      ...
      <label className="flex items-start gap-2 cursor-pointer">
        ...
        <span className="text-xs text-gray-100">{tx('我已閱讀授權要點與完整授權書(如附連結),同意授權範圍,確認中選後願意簽署正式授權文件。', 'I have read the key terms and the full (redacted) license letter, agree to the scope, and will sign the formal authorization if selected.')}</span>
      </label>
    </div>
  );
}
        ...
        {open && !closed && (
          <div className="space-y-2 border-t border-white/[0.08] pt-3">
            ...
            <div className="flex gap-2">
              {dealCurrency
                ? <span className={`${cls} w-20 inline-flex items-center justify-center text-gray-200`}>{dealCurrency}</span>
