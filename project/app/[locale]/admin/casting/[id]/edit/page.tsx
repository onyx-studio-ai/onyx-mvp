...
import { countryLabel } from '@/lib/talent-taxonomy';
...
export default function EditCasting() {
  ...
  const [talents, setTalents] = useState<{ id: string; name: string; email: string; active?: boolean; no?: number; realNames?: string[]; reach?: string; country?: string }[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setF({
      title: bf.title || '', internal_client_note: bf.internal_client_note || '', license_summary: bf.license_summary || '', content_type: bf.content_type || '', language: bf.language || '', brief: bf.brief || '',
              ...
              {assignMode === 'existing' ? (
                <div className="block">
                  ...
                  {(() => {
                    ...
                    const label = (t: { name: string; email: string; active?: boolean; no?: number; realNames?: string[]; reach?: string; country?: string }) =>
                      `${t.name}${t.no ? `(T-${t.no})` : ''} · ${t.email.split('@')[0]}${t.country ? ` · ${countryLabel(t.country, 'zh-TW')}` : ''}${t.realNames?.length ? ` · 真名:${t.realNames.join('/')}` : ''}${t.active === false ? ' · 未上線' : ''}${t.reach ? ` · ✓${t.reach}` : ' · ⚠僅email'}`;
