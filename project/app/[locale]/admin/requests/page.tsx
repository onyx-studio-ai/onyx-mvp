...
export default function AdminRequests() {
  ...
  async function setStatus(id: string, status: string) {
    ...
    try {
      ...
      if (!res.ok) { const j = await res.json().catch(() => ({})); toast.error(j.error || t('toastUpdateFail')); }
    } catch { toast.error(t('toastNetworkError')); } finally { setBusy(null); }
    load();
  }
  ...
  const stats = [
    { label: t('statPending'), value: requests.length },
    { label: t('statRequestedTalent'), value: requests.filter((b) => b.requested_talent).length, color: 'text-amber-700' },
    { label: t('statLiveSession'), value: requests.filter((b) => b.wants_live_session).length, color: 'text-sky-700' },
    { label: t('statHasSinging'), value: requests.filter((b) => b.has_singing).length, color: 'text-pink-700' },
  ];
  ...
  return (
    <div className="p-6 lg:p-8 text-gray-900">
      ...
      {filtered.length === 0 && <p className="text-gray-500 text-sm">{requests.length === 0 ? t('emptyNoRequests') : t('emptyNoMatch')}</p>}
        ...
        {filtered.map((b) => (
          <div key={b.id} className="bg-white border border-gray-200 shadow-sm rounded-xl p-5">
            <div className="flex items-start justify-between gap-3 mb-2">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[11px] bg-amber-100 text-amber-800 px-2 py-0.5 rounded-full">{t('badgePending')}</span>
                ...
                <p className="text-base font-semibold text-gray-900 truncate">{b.title || t('titleFallback', { type: b.content_type || t('defaultContentType') })}</p>
              </div>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 mb-3 text-sm">
              <p className="text-gray-900 font-medium">{b.client_name || t('dash')}{b.company ? ` · ${b.company}` : ''}</p>
            ...
            {(b.has_singing || b.wants_director || b.wants_live_session) && (
              <div className="flex flex-wrap gap-1.5 mb-3">
                ...
                {b.wants_live_session && <span className="text-xs bg-sky-100 text-sky-800 px-2 py-0.5 rounded-full">{t('flagLiveSession')}{b.live_session_tool ? ` · ${b.live_session_tool}` : ''}</span>}
              </div>
            )}
            ...
            {b.requested_talent && (
              <p className="text-sm mb-3 inline-flex items-center gap-1.5 bg-amber-50 border border-amber-200 text-amber-800 rounded-lg px-2.5 py-1">
                <span className="font-semibold">🎯 {t('requestedTalent')}</span> {b.requested_talent}
              </p>
            )}

            {/* full case data */}
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-5 gap-y-1.5 text-sm mb-3">
              {ROW(t('rowLanguage'), b.language)}
              {ROW(t('rowAccent'), b.accent)}
              {ROW(t('rowVoiceStyle'), b.voice_style)}
              {ROW(t('rowVoiceAge'), b.voice_age)}
              {ROW(t('rowVoicesNeeded'), b.voices_needed ? t('voicesCount', { count: b.voices_needed, plus: b.voices_needed >= 3 ? '+' : '' }) : null)}
              {ROW(t('rowGender'), b.gender_needs)}
              {ROW(t('rowBudget'), b.budget ? `${b.budget_type ? `${b.budget_type} ` : ''}${b.budget}${b.budget_unit ? ` / ${b.budget_unit}` : ''}` : null)}
              {ROW(t('rowMediaScope'), b.media_scope)}
              {ROW(t('rowTerritory'), b.territory)}
              {ROW(t('rowLicense'), b.license_term)}
              {ROW(t('rowLength'), b.length)}
              {ROW(t('rowLocalStudio'), b.local_studio_region)}
              {ROW(t('rowAuditionDeadline'), b.audition_deadline)}
              {ROW(t('rowRecordingStart'), b.recording_start)}
              {ROW(t('rowDeadline'), b.deadline)}
              {(() => {
                const links = (b.reference_links && b.reference_links.length ? b.reference_links : (b.ref_audio_url ? [b.ref_audio_url] : [])).filter(Boolean);
                return links.length ? ROW(t('rowReference'), <span className="inline-flex flex-col gap-0.5">{links.map((l, i) => <a key={i} href={l} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline break-all">{l}</a>)}</span>) : null;
              })()}
            </div>
            ...
            {(b.script_text || b.script_file_url) && (
              <div className="mb-3 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2">
                <p className="text-xs font-semibold text-gray-600 mb-1">
                  {b.script_type === 'final' ? t('scriptFinal') : b.script_type === 'audition' ? t('scriptAudition') : t('scriptGeneric')}
                  {b.script_file_url && <a href={b.script_file_url} target="_blank" rel="noopener noreferrer" className="ml-2 text-blue-600 hover:underline">{t('downloadFile')}</a>}
                </p>
            ...
            {b.roles_file_url && (
              <p className="text-sm mb-3 inline-flex items-center gap-1.5 bg-amber-50 border border-amber-200 text-amber-800 rounded-lg px-2.5 py-1">
                <span className="font-semibold">🎭 {t('rolesTable')}</span> <a href={b.roles_file_url} target="_blank" rel="noopener noreferrer" className="underline">{t('downloadRolesTable')}</a>
                <span className="text-amber-600">{t('rolesAutoImport')}</span>
              </p>
            )}

            <div className="flex flex-wrap gap-2 border-t border-gray-200 pt-3">
              <a href={`/admin/casting/new?from=${b.id}`} className="text-sm bg-green-600 hover:bg-green-500 text-white font-semibold rounded-lg px-3.5 py-1.5">{t('openCasting')}</a>
              <button onClick={() => setOpenThread(openThread === b.id ? null : b.id)} className="text-sm bg-gray-100 hover:bg-gray-200 text-gray-700 border border-gray-200 rounded-lg px-3.5 py-1.5">{openThread === b.id ? t('collapseMessages') : t('replyClient')}</button>
              <button onClick={() => setStatus(b.id, 'closed')} disabled={busy === b.id} className="text-sm bg-gray-100 hover:bg-gray-200 disabled:opacity-50 text-gray-600 border border-gray-200 rounded-lg px-3.5 py-1.5">{t('markHandled')}</button>
            </div>
...
function RequestThread({ briefId }: { briefId: string }) {
  ...
  async function send() {
    ...
    try {
      ...
      const j = await r.json().catch(() => ({}));
      if (!r.ok) { toast.error(j.error || t('toastSendFail')); return; }
      setText(''); setMsgs((m) => [...m, j.message]);
    } catch { toast.error(t('toastSendFailRetry')); } finally { setSending(false); }
  }
  return (
    <div className="mt-3 border-t border-gray-200 pt-3">
      <p className="text-xs text-gray-500 mb-2">{t('threadHint')}</p>
      <div className="space-y-2 mb-3 max-h-72 overflow-y-auto">
        {msgs.length === 0 && <p className="text-xs text-gray-400">{t('threadEmpty')}</p>}
        {msgs.map((m) => (
          <div key={m.id} className={`flex ${m.sender_type === 'admin' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] rounded-xl px-3 py-2 text-sm ${m.sender_type === 'admin' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-800'}`}>
              <div className={`text-[10px] mb-0.5 ${m.sender_type === 'admin' ? 'text-blue-100' : 'text-gray-500'}`}>{m.sender_name || (m.sender_type === 'admin' ? 'Onyx Studios' : t('senderClient'))} · {(m.created_at || '').slice(0, 16).replace('T', ' ')}</div>
      ...
      <div className="flex gap-2">
        <textarea value={text} onChange={(e) => setText(e.target.value)} rows={2} className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm resize-y" placeholder={t('threadPlaceholder')} />
        <button onClick={send} disabled={sending || !text.trim()} className="text-sm bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-semibold rounded-lg px-4 self-end py-2">{sending ? t('sending') : t('send')}</button>
      </div>
    </div>
  );
}
