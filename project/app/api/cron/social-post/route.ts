...
/*
  社群自動發文 cron(vercel.json 設定,每 10 分鐘跑一次)。

  規則:
    • 撈 social_queue 裡 status='ready' 且 scheduled_for 已到(或 null)的到期貼文,
      一次最多發 MAX_PER_RUN(5)筆 —— 讓使用者各自排的時間都能準時發,
      上限做洪水保護(積壓也不會一次噴光)。
...
export async function GET(request: NextRequest) {
  ...
  // 每次最多處理 MAX_PER_RUN 筆到期的(scheduled_for 已到或 null)。
  // cron 每 10 分鐘跑一次 → 各自排的時間都能準時發,不用一天等一則;
  // 上限做洪水保護,萬一佇列積壓也不會一次噴光。
  const MAX_PER_RUN = 5;
  ...
  if (due.length === 0) return NextResponse.json({ ok: true, posted: 0, note: '沒有到期的待發貼文' });

  if (dry) {
    return NextResponse.json({
      ok: true, dry: true, count: due.length,
      next: due.map((r) => ({ id: r.id, kind: r.kind, platforms: (r.platforms || []).filter(Boolean), text: r.text, link: r.link, media_url: r.media_url })),
    });
  }

  // 逐筆發送(每筆各自結算狀態)。單筆發一個平台的邏輯抽成 postRow。
  const summary: Array<{ id: string; kind: string; status: string; results: Record<string, unknown> }> = [];
  ...
  for (const row of due) {
    ...
    summary.push({ id: row.id, kind: row.kind, status, results });
  }

  const anyFail = summary.some((s) => s.status === 'failed');
  return NextResponse.json({ ok: !anyFail, processed: summary.length, results: summary });
}
