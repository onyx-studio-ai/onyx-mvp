...
/*
  ...
  Wing 打開 /admin/casting/new 按「恢復草稿」即全帶入 —— 發案管線零改動。
  需要 Vercel 環境變數 ANTHROPIC_API_KEY。
*/
...
const DRAFT_TOOL: Anthropic.Tool = {
  name: 'case_draft',
  description: '所有必備欄位收齊後呼叫,產出發案草稿。缺必備欄位(報酬未定、語言不明等)時不要呼叫,先追問。',
  input_schema: {
    type: 'object',
...
export async function POST(request: NextRequest) {
  ...
  if (!apiKey) return NextResponse.json({ error: 'ANTHROPIC_API_KEY 未設定 —— 請在 Vercel 環境變數加入後重新部署。' }, { status: 500 });

  let body: { messages?: { role: 'user' | 'assistant'; content: string }[] };
  try { body = await request.json(); } catch { return NextResponse.json({ error: 'Invalid JSON' }, { status: 400 }); }
  const msgs = (body.messages || []).filter((m) => m && (m.role === 'user' || m.role === 'assistant') && typeof m.content === 'string' && m.content.trim());
  ...
  try {
    ...
    if (tool) return NextResponse.json({ draft: tool.input });
    const text = resp.content.filter((b): b is Anthropic.TextBlock => b.type === 'text').map((b) => b.text).join('\n').trim();
    if (resp.stop_reason === 'refusal' || !text) return NextResponse.json({ reply: '這個需求我沒辦法處理,請換個說法或直接手動發案。' });
    return NextResponse.json({ reply: text });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'AI 呼叫失敗';
