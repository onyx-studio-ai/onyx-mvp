...
export async function GET(request: NextRequest) {
  ...
  const [{ data: brief }, { data: orders }] = await Promise.all([
    ...
    db.from('voice_orders')
      .select('id, order_number, role_name, talent_id, status, script_text, production_notes, reference_files, role_images, talent_price, price, pay_unit, pay_rate, currency, deadline, created_at, talents(name)')
      .eq('brief_id', briefId)
