...
export async function GET(request: NextRequest) {
  ...
  try {
    const { data: briefs, error: bErr } = await r.db
      ...
      .select('id, brief_number, kind, title, roles, audition_script, reference_links, reference_files, recording_start, recording_methods, rate_note, base_revisions, categories, content_type, media_scope, territory, license_term, script_status, has_singing, wants_director, wants_live_session, live_session_tool, audition_deadline, language, length, budget, budget_type, deadline, brief, created_at, status')
      .eq('status', 'open')
