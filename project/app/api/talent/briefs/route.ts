...
export async function GET(request: NextRequest) {
  ...
  try {
    const { data: briefs, error: bErr } = await r.db
      ...
      .select('id, brief_number, categories, content_type, media_scope, territory, license_term, script_status, has_singing, wants_director, wants_live_session, audition_deadline, language, length, budget, deadline, brief, created_at, status')
      .eq('status', 'open')
