...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (wonIds.length) {
      ...
      const { data: ords } = await r.db.from('voice_orders').select('id, brief_id, script_text, script_file_url, script_files, production_notes, reference_files, voice_sample_files, deadline, deadline_time, created_at, status, payment_status').in('brief_id', wonIds);
      const orderByBrief: Record<string, { id: string; script_text?: string | null; script_file_url?: string | null; script_files?: { name?: string; url: string }[] | null; production_notes?: string | null; reference_files?: { name?: string; url: string }[] | null; voice_sample_files?: { name?: string; url: string }[] | null; deadline?: string | null; deadline_time?: string | null; created_at?: string | null; status?: string | null; payment_status?: string | null }> = {};
      const orderIdToBrief: Record<string, string> = {};
      for (const o of ords || []) { if (o.brief_id) { orderByBrief[o.brief_id as string] = { id: o.id as string, script_text: o.script_text as string | null, script_file_url: o.script_file_url as string | null, script_files: (o.script_files as { name?: string; url: string }[] | null) || null, production_notes: o.production_notes as string | null, reference_files: (o.reference_files as { name?: string; url: string }[] | null) || null, voice_sample_files: (o.voice_sample_files as { name?: string; url: string }[] | null) || null, deadline: o.deadline as string | null, deadline_time: o.deadline_time as string | null, created_at: o.created_at as string | null, status: o.status as string | null, payment_status: o.payment_status as string | null }; orderIdToBrief[o.id as string] = o.brief_id as string; } }
      ...
      wonBriefs = (wb || []).map((b) => ({ ...b, order_id: orderByBrief[(b as { id: string }).id]?.id || null, order_status: orderByBrief[(b as { id: string }).id]?.status || null, order_payment_status: orderByBrief[(b as { id: string }).id]?.payment_status || null, final_script: orderByBrief[(b as { id: string }).id]?.script_text || null, final_script_url: orderByBrief[(b as { id: string }).id]?.script_file_url || null, final_script_files: orderByBrief[(b as { id: string }).id]?.script_files || [], order_production_notes: orderByBrief[(b as { id: string }).id]?.production_notes || null, order_voice_samples: orderByBrief[(b as { id: string }).id]?.voice_sample_files || [], order_reference_files: orderByBrief[(b as { id: string }).id]?.reference_files || [], order_deadline_time: orderByBrief[(b as { id: string }).id]?.deadline_time || null, order_deadline: orderByBrief[(b as { id: string }).id]?.deadline || null, order_created: orderByBrief[(b as { id: string }).id]?.created_at || null, deliveries: deliveriesByBrief[(b as { id: string }).id] || [] }));
    }
